...
| 助手                                                             | 用途                                                               |
| ---------------------------------------------------------------- | ------------------------------------------------------------------ |
| `{{#if condition}}...{{else if condition}}...{{else}}...{{/if}}` | 条件渲染                                                           |
| `{{#unless condition}}...{{/unless}}`                            | 反向条件渲染（condition 为假时渲染）                               |
| `{{#each list}}...{{/each}}`                                     | 遍历数组，当前元素为 `{{this}}`，可访问 `{{@index}}` 和 `{{@key}}` |
| `{{#with object}}...{{/with}}`                                   | 将上下文切换到指定对象                                             |
| `{{lookup object key}}`                                          | 动态属性访问（key 可以是变量）                                     |
| `{{log value}}`                                                  | 输出日志，调试用                                                   |
