...
`#` 标题到第一个 `##`/`###` 之间的内容属于**故事模板**，会在故事开始时渲染一次。`##` 标题到第一个 `###` 之间的内容属于**章节模板**，会在每次进入该章节时渲染一次。两者都支持 Handlebars 语法和内置助手函数（`{{input}}`、`{{#nav}}`、`{{linebreak}}` 等）。
...
YAML metadata 中定义资源，资源会被展开到模板作用域中，通过键名以对象形式直接访问：
